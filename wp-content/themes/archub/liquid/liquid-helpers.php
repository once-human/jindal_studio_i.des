...
class Liquid_Helper {
	...
	/**
	 * Instantiates the WordPress filesystem for use with archub.
	 *
	 * @static
	 * @access public
	 * @return object
	...
	public function get_template_part( $template, $args = null ) {
		...
		if ( ! file_exists( $located ) ) {
			_doing_it_wrong( __FUNCTION__, sprintf( wp_kses( __( '<code>%s</code> does not exist.', 'archub' ), array( 'code' => array() ) ), $located ), null );
			return;
		}
	...
	public function tgmpa_plugin_action( $plugin, $status ) {
		...
		switch( $status ) {
			case 'not-installed':
				$btn_class = 'white';
				$btn_text = esc_html_x( 'Install', 'Liquid plugin installation page.', 'archub' );
			...
			case 'installed':
				$btn_class = 'success';
				$btn_text = esc_html_x( 'Activate', 'Liquid plugin installation page.', 'archub' );
			...
			case 'active':
				$btn_class = 'danger';
				$btn_text = esc_html_x( 'Deactivate', 'Liquid plugin installation page.', 'archub' );
	...
	public function liquid_post_date() {
		...
		if ( liquid_helper()->get_theme_option( 'blog-date-format' ) === 'ago' ) {
			return sprintf( esc_html__( '%s ago', 'archub' ), human_time_diff( get_the_time( 'U' ), current_time( 'timestamp' ) ) );
		} else{
			return get_the_date();
		}
