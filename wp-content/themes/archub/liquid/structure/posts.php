...
function liquid_attributes_post( $attributes ) {
	...
	if ( ! empty( $post ) ) {
		...
		if( !class_exists( 'ReduxFramework' ) || !class_exists( 'Liquid_Addons' ) ) {
			$class = is_singular() ? 'text-start pos-rel' : 'lqd-lp-style-11 lqd-lp-hover-img-zoom text-start pos-rel';
			$attributes['class'] = join( ' ', get_post_class( array( $attributes['class'], $class ) ) );
		}
		else {
...
function liquid_attributes_entry_published( $attributes ) {
	...
	$attributes['title']    = get_the_time( esc_html_x( 'l, F j, Y, g:i a', 'post time format', 'archub' ) );
...
function liquid_get_post_terms( $args ) {
	...
	$defaults = array(
		...
		'sep'        => esc_html_x( ', ', 'taxonomy terms separator', 'archub' )
	);
	$args = wp_parse_args( $args, $defaults );

	$classnames = 'lqd-lp-cat lqd-lp-cat lqd-lp-cat-shaped lqd-lp-cat-solid reset-ul inline-nav d-flex flex-wrap pos-rel z-index-3 font-weight-bold text-uppercase ltr-sp-1 ';

	if ( $args['solid'] ) {
		$classnames .= 'lqd-lp-cat-shaped lqd-lp-cat-solid lqd-lp-cat-solid-colored';
	}
	
	$terms = get_the_terms( $args['post_id'], $args['taxonomy'] );

	if ( $terms ) {
		$out .= sprintf( $args['before'], liquid_helper()->get_attr( 'entry-terms', array( 'taxonomy' => $args['taxonomy'] ) ) );
		$out .= '<ul class="' . esc_attr($classnames) . '">';
		foreach ($terms as $term) {
			$out .= '<li class="' . $args['taxonomy'] . '-' . $term->slug . '">';
			$out .= '<a class="lh-15 circle" href="' . get_term_link( $term->slug, $args['taxonomy']) . '" rel="' . $args['taxonomy'] . '">';
			$out .= $term->name;
			$out .= '</a>';
			$out .= '</li>';
		}
...
function liquid_post_title_description() {
	...
	elseif ( is_search() )
		$title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'archub' ), get_search_query() );
	...
	elseif ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
		$title = get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'archub' ) );

	elseif ( get_query_var( 'minute' ) )
		$title = sprintf( esc_html__( 'Minute %s', 'archub' ), get_the_time( esc_html_x( 'i', 'minute archives time format', 'archub' ) ) );

	elseif ( get_query_var( 'hour' ) )
		$title = get_the_time( esc_html_x( 'g a', 'hour archives time format', 'archub' ) );

	elseif ( is_day() )
		$title = get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'archub' ) );

	elseif ( get_query_var( 'w' ) )
		$title = sprintf( esc_html__( 'Week %1$s of %2$s', 'archub' ), get_the_time( esc_html_x( 'W', 'weekly archives date format', 'archub' ) ), get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'archub' ) ) );

	elseif ( is_month() )
		$title = single_month_title( ' ', false );

	elseif ( is_year() )
		$title = get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'archub' ) );

	elseif ( is_archive() )
		$title = hesc_html__( 'Archives', 'archub' );
