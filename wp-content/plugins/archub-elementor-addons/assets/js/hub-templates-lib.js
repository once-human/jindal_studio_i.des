;( function( elementor, $, window ) {
	...
	var TemplateLibraryInsertTemplateBehavior = Marionette.Behavior.extend({
		...
		onInsertButtonClick: function onInsertButtonClick() {
			...
			$e.run('hub_collections/insert-template', args);
		}
	});
	...
	var TemplateLibraryTemplateHubView = TemplateLibraryTemplateView.extend({
		template: '#tmpl-elementor-template-library-template-hub',
		ui: function ui() {
		...
		onPreviewButtonClick: function onPreviewButtonClick() {
			$e.route('hub_collections/preview', {
				model: this.model
			});
		},
		onFavoriteCheckboxChange: function onFavoriteCheckboxChange() {
			...
			hub_templates_lib.templates.markAsFavorite(this.model, isFavorite);

			if (!isFavorite && hub_templates_lib.templates.getFilter('favorite')) {
	...
	var TemplateLibraryHeaderPreviewView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-preview-hub',
	...
	var TemplateLibraryHeaderBackView = Marionette.ItemView.extend( {
		...
		onClick: function() {
			$e.routes.restoreState( 'hub_collections' );
		},
	} );

	var TemplateLibraryCollectionView = Marionette.CompositeView.extend({
		template: '#tmpl-elementor-template-library-templates-hub',
		...
		getChildView: function getChildView(childModel) {
			return TemplateLibraryTemplateHubView;
		},
		initialize: function initialize() {
			this.listenTo(elementor.channels.templates, 'filter:change', this._renderChildren);
		},
		filter: function filter(childModel) {
			var filterTerms = hub_templates_lib.templates.getFilterTerms(),
				passingFilter = true;
			jQuery.each(filterTerms, function (filterTermName) {
				var filterValue = hub_templates_lib.templates.getFilter(filterTermName);
		...
		addSourceData: function addSourceData() {
			...
			this.$el.attr('data-template-source', isEmpty ? 'empty' : hub_templates_lib.templates.getFilter('source'));
		},
		...
		toggleFilterClass: function toggleFilterClass() {
			this.$el.toggleClass('elementor-templates-filter-active', !!(hub_templates_lib.templates.getFilter('text') || hub_templates_lib.templates.getFilter('favorite')));
		},
		onRender: function onRender() {
			if ('hub_collections' === hub_templates_lib.templates.getFilter('source')) {
				this.setFiltersUI();
			}
		},
		onRenderCollection: function onRenderCollection() {
			...
			if ('hub_collections' === hub_templates_lib.templates.getFilter('source') && 'page' !== hub_templates_lib.templates.getFilter('type')) {
				this.setMasonrySkin();
			}
		},
		...
		onTextFilterInput: function onTextFilterInput() {
			hub_templates_lib.templates.setFilter('text', this.ui.textFilter.val());
		},
		onSelectFilterChange: function onSelectFilterChange(event) {
			var $select = jQuery(event.currentTarget),
				filterName = $select.data('elementor-filter');
			hub_templates_lib.templates.setFilter(filterName, $select.val());
		},
		onMyFavoritesFilterChange: function onMyFavoritesFilterChange() {
			hub_templates_lib.templates.setFilter('favorite', this.ui.myFavoritesFilter[0].checked);
		},
	...
	var TemplateLibraryModalLogoView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-templates-modal__header__logo_hub',
		className: function className() {
	...
	var TemplateLibraryHeaderActionsView = Marionette.ItemView.extend({
		template: '#tmpl-elementor-template-library-header-actions-hub',
		...
		onSyncClick: function() {
			...
			hub_templates_lib.templates.requestLibraryData({
				onUpdate: function onUpdate() {
					self.ui.sync.removeClass('eicon-animation-spin');
					$e.routes.refreshContainer('hub_collections');
				},
				forceUpdate: true,
				forceSync: true
			});
	...
	var TemplateLibraryHeaderMenuView = Marionette.ItemView.extend({
		...
		templateHelpers: function templateHelpers() {
			return {
				tabs: $e.components.get('hub_collections').getTabs()
			};
		}
	});
	...
	var TemplateLibraryLayoutView = elementorModules.common.views.modal.Layout.extend({
		...
		getLogoOptions: function() {
			return {
				title: 'ArcHub Collections',
				click: function click() {
					$e.run('hub_collections/open', {
						toDefault: true
					});
					return false;
				}
		...
		getTemplateActionButton: function(templateData) {
			...
			if ( templateData.isPro && 'inactive' === templateData.proStatus ) {
				viewId = '#tmpl-elementor-template-library-get-pro-buttohub';
			}
			if ( templateData.isPro && 'license_inactive' === templateData.proStatus ) {
				viewId = '#tmpl-elementor-pro-template-library-activate-license-button-hub';
			}
	...
	var Component = function(ComponentModal) {
		...
		Component.prototype.__construct = function(args) {
			...
			$e.data.deleteCache( this, 'hub_collections' );
		};
		...
		Component.prototype.defaultTabs = function() {
			return {
				'templates/blocks': {
					...
					getFilter: function() {
						return {
							source: 'hub_collections',
							type: 'block',
							// causing empty blocks in elementor templates pages
		...
		Component.prototype.defaultCommands = function() {
			return Object.assign( parent.defaultCommands.call( this ), {
				...
				'insert-template': this.insertTemplate.bind(this)
			} );
		};
		...
		Component.prototype.activateTab = function(tab) {
			$e.routes.saveState('hub_collections');
		...
		Component.prototype.show = function( args ) {
			...
			if ( args.toDefault || ! $e.routes.restoreState( 'hub_collections' ) ) {
		...
		Component.prototype.maybeOpenLibrary = function() {
			if ('#hub_collections' === location.hash) {
				$e.run('hub_collections/open');
				location.hash = '';
			}
		};
	...
	var TemplateLibraryManager = function TemplateLibraryManager() {
		...
		this.requestLibraryData = function (options) {
			...
			elementorCommon.ajax.addRequest('hub_get_library_data', ajaxOptions);
		};
		...
		this.filterTemplates = function () {
			//var activeSource = 'hub_collections';
			return templatesCollection.filter(function (model) {
				if ('hub_collections' !== model.get('source')) {
					return false;
				}
	...
	hub_templates_lib.templates = new TemplateLibraryManager();
	hub_templates_lib.templates.init();
	...
	elementor.on('document:loaded', function() {
		...
		var $templateLibBtnStyle = $( '<style />' ),
			btnStyle = '';
			btnStyle += '.elementor-add-section-area-button.hub-add-template-button { margin-left: 8px; vertical-align: bottom; }';
			btnStyle += '.elementor-add-section-area-button.hub-add-template-button img { height: 40px; padding: 5px; }';
		...
		$templateLibBtn.addClass( 'elementor-add-section-area-button hub-add-template-button' );
		$templateLibBtn.attr( 'title', 'Hub Collections' );
		$templateLibBtn.html( '<img src="' + hub_templates_lib.logoUrl + '" />' );
		
		$templateLibBtn.on( 'click', () => {
			$e.run('hub_collections/open', { importOptions: { at: undefined } });
		});

		$templateLibBtn.insertAfter( $previewContents.find('.elementor-add-section-area-button.elementor-add-template-button') );

		$previewContents.find('body').delegate('.elementor-editor-element-add', 'click', ev => {
			const timeout = setTimeout(() => {
				
				const $currentTarget = $(ev.currentTarget);
				const $parentSection = $currentTarget.closest('.elementor-element');
				const $addSectionSection = $parentSection.prev('.elementor-add-section-inline');
				const $btnClone = $templateLibBtn.clone(false);

				$btnClone.on( 'click', () => {

					const sectionModelId = $parentSection.data('model-cid');
					const modelIndex = elementor.elements.findIndex(el => el['cid'] === sectionModelId);

					const importOptions = {};

					if ( modelIndex > -1 ) {
						importOptions['at'] = modelIndex;
					}

					$e.run('hub_collections/open', { importOptions });

				});

				$btnClone.insertAfter( $addSectionSection.find('.elementor-add-section-area-button.elementor-add-template-button') );

				$addSectionSection.find('.elementor-add-section-close').on('click', () => {
