...
function liquid_likes_button( $args = array(), $post_id = 0 ) {
	...
	if( !post_type_supports( get_post_type( $post_id ), 'liquid-post-likes' ) ) {
		echo __( 'Post type not support likes.', 'archub-core' );
		return;
	}
...
class Liquid_Post_Like extends Liquid_Base {
	...
	public function save_like() {
		...
		if ( ! wp_verify_nonce( $nonce, $this->noncekey ) ) {
			wp_send_json_error( esc_html__( 'Not permitted', 'archub-core' ) );
		}

		// Get post_id
		$post_id = isset( $_REQUEST['post_id'] ) && is_numeric( $_REQUEST['post_id'] ) ? $_REQUEST['post_id'] : '';
		if( ! $post_id ) {
			wp_send_json_error( esc_html__( 'No Post ID', 'archub-core' ) );
		}

		// if post support post likes
		if( !post_type_supports( get_post_type( $post_id ), 'liquid-post-likes' ) ) {
			wp_send_json_error( esc_html__( 'Post type not support likes.', 'archub-core' ) );
		}
	...
	public function get_likes_button( $args = array(), $post_id = 0 ) {
		...
		$defaults = array(
			...
			'format' => wp_kses_post( __( '<span class="post-likes-count">%s</span> Likes', 'archub-core' ) ),
		);
