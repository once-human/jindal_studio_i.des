...
if( class_exists( 'ReduxFramework' ) ) {
	...
	class Liquid_Custom_Fonts {
		...
		function add_font_face_css( $css ) {
			...
			$font_display = liquid_helper()->get_theme_option('custom_fonts_display');
		...
		function add_font_preload_links() {
			...
			$woff_arr  = $this->liquid_custom_fonts_woff;
			$ttf_arr   = $this->liquid_custom_fonts_ttf;
			$preload   = liquid_helper()->get_theme_option('preload_liquid_custom_fonts') === 'on' ? 'rel=preload' : '';
			...
			foreach( $get_fonts as $key => $font_name ) {
				...
				if( isset( $woff2_arr[ $key ] ) && ! empty( $woff2_arr[ $key ] ) ) {					
					$out .= '<link ' . esc_attr($preload) . ' href="' . esc_url( $woff2_arr[ $key ] ) . '" as="font" type="font/woff2" crossorigin>';
				}
				if( isset( $woff_arr[ $key ] ) &&  ! empty( $woff_arr[ $key ] ) ) {
					$out .= '<link ' . esc_attr($preload) . ' href="' . esc_url( $woff_arr[ $key ] ) . '" as="font" type="font/woff" crossorigin>';
				}
				if( isset( $ttf_arr[ $key ] ) && ! empty( $ttf_arr[ $key ] ) ) {
					$out .= '<link ' . esc_attr($preload) . ' href="' . esc_url( $ttf_arr[ $key ] ) . '" as="font" type="font/ttf" crossorigin>';
				}
